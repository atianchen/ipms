    ...
    export default {
        ...
        methods: {
            itemClick: function (param) {
                this.$router.push({ name:'accuralForecast', params: { projId: param }})
            },
            ...
            {
                let _self=this;
                $.post("/im/listPmForecastProject",{q:util.qfilter(this.q),page:this.page}).done((rs)=>{_self.gridData=rs.data;_self.page=rs.page;}).fail(function(){})
            }
        },
        components: {
            'hy-grid': Grid,
